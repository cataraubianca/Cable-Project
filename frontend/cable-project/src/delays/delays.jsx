...
export const Delays = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true); 
    try {
      const response = await fetch('http://127.0.0.1:5000/dw', {
        method: 'POST',
        headers: {
  ...
  return (
    ...
    <div className={styles['form-container']}>
      <h2>See just the delays between the source and other countries in the given time</h2>
      <form onSubmit={handleSubmit}>
        ...
        {isLoading ? (
            ...
            <button type="button" className={styles['submit-button']} onClick={() => history('/dw')}>See Results</button>
          ) : (
